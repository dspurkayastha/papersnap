...
def _detect_chandra() -> Tuple[bool, Optional[str]]:
    return _load_module("chandra_ocr")


def _detect_surya() -> Tuple[bool, Optional[str]]:
    return _load_module("surya_ocr")
...
def run_chandra(file_path: Path) -> Optional[Dict[str, Any]]:
    if not (engine_states["chandra"].enabled and engine_states["chandra"].available):
        return None
    try:
        from chandra_ocr import pipeline as chandra_pipeline  # type: ignore

        predictor = None
        if hasattr(chandra_pipeline, "Pipeline"):
            predictor = chandra_pipeline.Pipeline()  # type: ignore[attr-defined]
        elif hasattr(chandra_pipeline, "load_pipeline"):
            predictor = chandra_pipeline.load_pipeline()
        if predictor is None and hasattr(chandra_pipeline, "ChandraOCR"):
            predictor = chandra_pipeline.ChandraOCR()  # type: ignore[attr-defined]

        if predictor is None:
            raise RuntimeError("Unsupported chandra_ocr API")

        if hasattr(predictor, "ocr"):
            output = predictor.ocr(str(file_path))  # type: ignore[misc]
        elif hasattr(predictor, "__call__"):
            output = predictor(str(file_path))
        else:
            raise RuntimeError("Chandra pipeline missing callable interface")
...
def run_surya(file_path: Path) -> Optional[Dict[str, Any]]:
    if not (engine_states["surya"].enabled and engine_states["surya"].available):
        return None
    try:
        from surya_ocr import pipeline as surya_pipeline  # type: ignore

        predictor = None
        if hasattr(surya_pipeline, "Pipeline"):
            predictor = surya_pipeline.Pipeline()  # type: ignore[attr-defined]
        elif hasattr(surya_pipeline, "load_pipeline"):
            predictor = surya_pipeline.load_pipeline()
        if predictor is None and hasattr(surya_pipeline, "SuryaOCR"):
            predictor = surya_pipeline.SuryaOCR()  # type: ignore[attr-defined]

        if predictor is None:
            raise RuntimeError("Unsupported surya_ocr API")
        ...
        if hasattr(predictor, "ocr"):
            ...
            raise RuntimeError("Surya pipeline missing callable interface")
...
def run_gcv(file_path: Path) -> Optional[Dict[str, Any]]:
    global gcv_client
    if not (engine_states["gcv"].enabled and engine_states["gcv"].available):
        return None
    try:
        ...
        meta = {
            "locale": response.full_text_annotation.pages[0].property.detected_languages[0].language_code
...
def run_deepseek(file_path: Path) -> Optional[Dict[str, Any]]:
    if not (engine_states["deepseek"].enabled and engine_states["deepseek"].available):
        return None
    url = os.getenv("DEEPSEEK_OCR_URL")
    if not url:
        return None
...
SURGERY_KEYWORDS = ["diagnosis", "procedure", "surgeon", "laparotomy", "peritonitis", "ot note"]
...
def parse_surgery_fields(raw_text: str) -> Dict[str, Any]:
    ...
    date_match = re.search(
        r"(?:surgery date|date of surgery|dos|operation date)[:\-\s]*([0-9]{4}[\-/][0-9]{2}[\-/][0-9]{2}|[0-9]{2}[\-/][0-9]{2}[\-/][0-9]{4})",
        raw_text,
        flags=re.IGNORECASE,
    )
    if date_match:
...
def collect_engine_results(file_path: Path) -> List[Dict[str, Any]]:
    ...
    for engine_id in ENGINE_ORDER:
        ...
        except Exception as exc:  # pragma: no cover - runner safeguards
...
async def health() -> JSONResponse:
    refresh_engine_states()
    return JSONResponse({"status": "ok", "engines": [_serialize_engine_state(state) for state in engine_states.values()]})


@app.get("/engines")
async def list_engines() -> JSONResponse:
    refresh_engine_states()
    return JSONResponse({"engines": [_serialize_engine_state(state) for state in engine_states.values()]})
...
async def analyze(request: AnalyzeRequest) -> JSONResponse:
    ...
    if not results:
        ...
        if stub_state and stub_state.enabled:
            logger.info("Falling back to stub OCR for %s", request.document_id or file_path.name)
